...
# --- Rate Limiting ---
# Key function: use API key if present, otherwise fall back to IP
...
async def chat(request: Request, chat_request: ChatRequest):
    ...
    try:
        ...
        history_list = [(m.role, m.content) for m in chat_request.history]

        # Pass country code to engine
        result = engine.query(chat_request.query, history=history_list, country_code=chat_request.country)
...
async def get_latest_evals(request: Request):
    """
